...
public class Ai {
	...
	private static List<Map<Integer, Symbol>> patterns; // all possible winning triplets
	...
	protected static int aiMove(int userMove) {
		if (freeToMoveCells.contains(userMove)) {
			...
			fillUserMoveInPatterns(userMove);
			userMoves++;
		}
		else {
	...
	6 - go to a random
	 */
	private static int aiImpossible(int userMove) { // parameter not needed?
		Integer aiMove = null;
	...
	private static void fillUserMoveInPatterns(int moveIndex) {
		for (Map<Integer, Symbol> map : patterns) {
			if (map.containsKey(moveIndex)) {
				map.put(moveIndex, userSymbol);
			}
		}
	}
	...
	private static boolean userStartedOppositeCorners() {
		boolean bool = false;
		if (firstTwoMoves[0] == getOppositeCorner(firstTwoMoves[1]) || firstTwoMoves[1] == getOppositeCorner(firstTwoMoves[0])) {
			bool = true;
		}
		return bool;
	}
	...
	private static void deleteBlockedPatterns(int aiMove) {
		for (int i = 0; i < patterns.size(); i++) {
			if (patterns.get(i).containsKey(aiMove)) {
